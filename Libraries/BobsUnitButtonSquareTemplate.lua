...
function BobsUnitButtonSquareTemplate:Layout(button)
	...
	-- Overlay
	Graphics.Overlay:SetTexture(BobbyCode.Texture.SquareOverlay);
	Graphics.Overlay:ClearAllPoints();
	Graphics.Overlay:SetAllPoints(Graphics);
	Graphics.Overlay:Show();

	-- Border 
	Graphics.Border:SetTexture(BobbyCode.Texture.SquareSelect);
	...
	-- Aggro 
	Graphics.Aggro:SetTexture(BobbyCode.Texture.SquareSelect);
...
function BobsUnitButtonSquareTemplate:UpdateName(button)
	...
	button.Graphics.Name:SetText(strsub(UnitName(button.Unit), 0, 3));
end
